...
urlpatterns = [
  ...
  #  url(r'^uploader/media/$', views.simple_upload, name='simple_upload'),
   ...
   path('questions/', home_screen_view, name="home"),
    path('questions-account/', account_view, name="account"),
    path('questions/admin/', admin.site.urls),
    path('question/', include('blog.urls', 'blog')),
    path('questions-account/login/', login_view, name="login"),
    path('questions-account/logout/', logout_view, name="logout"),
	path('questions-account/must_authenticate/', must_authenticate_view, name="must_authenticate"),
    path('questions-account/register/', registration_view, name="register"),
    path('questions-account/password_change/done/', auth_views.PasswordChangeDoneView.as_view(template_name='registration/password_change_done.html'), 
        name='password_change_done'),

    path('questions-account/password_change/', auth_views.PasswordChangeView.as_view(template_name='registration/password_change.html'), 
        name='password_change'),

    path('questions-account/password_reset/done/', auth_views.PasswordResetCompleteView.as_view(template_name='registration/password_reset_done.html'),
     name='password_reset_done'),

    path('questions-account/reset/<uidb64>/<token>/', auth_views.PasswordResetConfirmView.as_view(), name='password_reset_confirm'),
    path('questions-account/password_reset/', auth_views.PasswordResetView.as_view(), name='password_reset'),
    
    path('questions-account/reset/done/', auth_views.PasswordResetCompleteView.as_view(template_name='registration/password_reset_complete.html'),
